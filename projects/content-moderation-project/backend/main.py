...
obs_manager: ObservabilityManager = None
...
async def lifespan(app: FastAPI):
    ...
    global db, auth_db, workflow, ml_status, obs_manager
    ...
    obs_manager = ObservabilityManager(
        service_name="content-moderation-system",
        log_level=LogLevel.INFO
    )
    obs_manager.logger.info("Observability system initialized")
    ...
    logger.info(f"   Fast Mode: {'[OK] Enabled' if fast_mode_enabled else '[X] Disabled'}")
    ...
    try:
        ...
        if final_state.get("policy_violations"):
            db.save_policy_violations(
                ...
                severity=final_state.get("violation_severity") or "none",
...
async def observability_health():
    ...
    return {
        "status": "healthy",
        "telemetry_enabled": obs_manager.telemetry.enabled if obs_manager else False,
        "log_count": len(obs_manager.logger.logs) if obs_manager else 0,
        "metric_count": len(obs_manager.performance.metrics) if obs_manager else 0,
        "timestamp": datetime.now().isoformat()
    }
...
async def observability_dashboard():
    ...
    return obs_manager.get_dashboard_data()
...
async def get_observability_logs(
    ...
    """Get filtered observability logs."""
    if not obs_manager:
        raise HTTPException(status_code=503, detail="Observability not initialized")
    ...
    if level:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid log level: {level}. Use DEBUG, INFO, WARNING, ERROR, or CRITICAL")

    logs = obs_manager.logger.get_logs(
        level=log_level,
...
async def get_observability_metrics(
    ...
    metrics = obs_manager.performance.get_metrics_summary(
