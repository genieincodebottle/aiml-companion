...
-- it just puts `src/` on the path and calls the same CLI. Use this if the
`triage` console script is not on your PATH, or if `pip install -e .` failed
(which it does on some Windows setups, with a stale entry-point .exe).
...
if __name__ == "__main__":
    try:
        from support_ticket_triage.cli import main
