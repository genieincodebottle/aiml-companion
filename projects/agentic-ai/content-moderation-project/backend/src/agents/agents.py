...
"""
            ...
            # Derive violation severity from toxicity score
            if toxicity_score >= 0.8:
                state["violation_severity"] = "high"
            elif toxicity_score >= 0.6:
                state["violation_severity"] = "medium"
            elif toxicity_score >= 0.3:
                state["violation_severity"] = "low"
            else:
                state["violation_severity"] = "none"
            ...
            # Log results
            logger.info(f"\n[OK] Fast Mode Decision: {decision.upper()}")
            ...
            logger.error(f"[FAIL] Failed to parse LLM response: {json_err}")
            ...
            logger.error(f"[FAIL] Fast Mode Agent Error: {e}")
            import traceback
            traceback.print_exc()
            # Fallback to safe default
            state["status"] = ContentStatus.FLAGGED.value
