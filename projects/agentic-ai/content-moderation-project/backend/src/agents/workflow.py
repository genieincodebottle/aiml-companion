...
def should_use_fast_mode(state: ContentState) -> bool:
    ...
    logger.info(f"[OK] Fast mode eligible: type={content_type}, length={len(content_text)} chars")
    return True
    ...
    try:
        if guardrail_manager or learning_tracker:
            ...
            if enable_fast_mode:
                ...
                logger.info("   [OK] Fast mode agent added")
        else:
        ...
        logger.error(f"   [FAIL] Failed to add node: {node_error}")
        import traceback
        traceback.print_exc()
        raise node_error
