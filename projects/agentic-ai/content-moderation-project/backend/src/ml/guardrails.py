...
class PromptInjectionDetector:
    """
    Detect prompt injection attempts in user-submitted content.

    Scans for common adversarial patterns that attempt to override
    system instructions, extract prompts, or manipulate agent behavior.
    This is the #1 security risk for LLM agents (OWASP LLM Top 10).
    """
    ...
    INJECTION_PATTERNS = [
        ...
        re.compile(r"override\s+(system|safety|content)\s+", re.IGNORECASE),
    ]

    def __init__(self):
        self.violations: List[GuardrailViolationRecord] = []

    def check_for_injection(self, content_text: str) -> Dict[str, Any]:
        """
        Scan content for prompt injection patterns.

        Args:
            content_text: User-submitted content to check
        ...
        if detected_patterns:
            self.violations.append(GuardrailViolationRecord(
                violation_type=GuardrailViolation.RATE_LIMIT_VIOLATED,  # Reuse enum for now
                message=f"Potential prompt injection detected: {detected_patterns}",
                timestamp=datetime.now(),
                metadata={"patterns": detected_patterns, "content_preview": content_text[:200]},
                severity="critical"
            ))
        ...
        return {
            ...
            "should_block": len(detected_patterns) > 0
        }
...
class GuardrailManager:
    ...
    def check_all_guardrails(
        ...
        # Check for prompt injection in content BEFORE any LLM processing
        content_text = content_state.get("content_text", "")
        if content_text:
            injection_result = self.injection_detector.check_for_injection(content_text)
            if injection_result["injection_detected"]:
                results["passed"] = False
                results["violations"].append(
                    f"Prompt injection detected: {injection_result['patterns_found']}"
                )
        ...
        all_violations = (
            ...
            self.injection_detector.violations
        )
