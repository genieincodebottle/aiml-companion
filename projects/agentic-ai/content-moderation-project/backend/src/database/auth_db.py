...
try:
    ...
    import warnings
    warnings.warn(
        "bcrypt not installed. Falling back to SHA-256 (NOT secure for production). "
...
class AuthDatabase:
    ...
    def authenticate_user(self, username: str, password: str) -> Optional[Dict]:
        ...
        try:
            ...
            cursor.execute('''
                SELECT user_id, username, full_name, role, email, phone, is_active, password_hash
                FROM users
            ...
            if not row or not self.verify_password(password, row['password_hash']):
                return None
            user = dict(row)
            del user['password_hash']  # Never expose hash
