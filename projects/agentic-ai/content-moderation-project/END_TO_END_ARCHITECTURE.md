...
**Usage Locations**:
1. **Content Analysis Agent** ([agents.py:58](backend/src/agents/agents.py#L58))
   - Topic extraction
   - Sentiment analysis
   - Content categorization
   - Uses structured prompts with JSON output parsing

2. **Toxicity Detection Agent** ([agents.py:178](backend/src/agents/agents.py#L178))
   - Language-aware toxicity assessment
   - Contextual hate speech detection
   - Cultural nuance understanding

3. **Policy Compliance Agent** ([agents.py:298](backend/src/agents/agents.py#L298))
   - Community guidelines checking
   - Policy violation categorization
   - Severity level determination

4. **Context Analysis Agent** ([agents.py:418](backend/src/agents/agents.py#L418))
   - User history analysis
   - Behavioral pattern detection
   - Risk level assessment

5. **Decision Synthesis Agent** ([agents.py:538](backend/src/agents/agents.py#L538))
   - ReAct reasoning loop (Think-Act-Observe)
   - Multi-agent decision aggregation
   - Final decision with confidence scoring

6. **Appeal Review Agent** ([agents.py:658](backend/src/agents/agents.py#L658))
...
**Implementation** ([memory.py:45-60](backend/src/memory/memory.py#L45-L60)):
```python
...
**Supported Models** ([ml_classifier.py:25-45](backend/src/ml/ml_classifier.py#L25-L45)):
...
**Default Mode**: Keyword-based detection ([keyword_detectors.py](backend/src/ml/keyword_detectors.py))
...
4. Entry Router Decision
   ...
   └─> Route: "content_analysis" (Full Pipeline)
       [WARN]  Story too long for fast mode - using full analysis
...
8. Agent 4: ReAct Decision Loop (Synthesis)
   ...
   │    [FAIL] Low confidence (<70%)? No (92%)                  │
   │    [FAIL] High severity violation? No                      │
   │    [FAIL] Conflicting decisions (<60% consensus)? No       │
   │    [FAIL] High-profile user? No                            │
   │    [FAIL] Sensitive content? No                            │
   │    [FAIL] Potential false positive? No                     │
   │    [FAIL] First offense + severe? No                       │
   │    [FAIL] Legal concern? No                                │
   │                                                        │
   │ 7. HITL decision                                       │
   │    hitl_required = False                               │
...
Final Status: Approved [OK]
User Impact: Story published immediately
```

---

### [ERR] Story 2: User Submits Toxic Comment (Removal Path)
...
2. Fast Mode Check
   ├─> len(content) = 68 chars < 200 [OK]
   ├─> content_type = "story_comment" [OK]
...
Decision: Removed + 7-day suspension [FAIL]
```
...
3. ReAct Decision Loop
   ...
   │                                                        │
   │ [OK] Low confidence: 0.68 < 0.70                          │
   │ [OK] Conflicting decisions: consensus 33% < 60%           │
   │ [OK] Sensitive content: political topic                   │
   │                                                        │
...
HITL ensures quality on edge cases [OK]
```
...
Outcome: Appeal upheld, content restored [OK]
...
### Data Privacy
- User passwords: bcrypt hashed with salt (stored in Auth DB)
...
- [OK] Scalable multi-agent processing
- [OK] Human oversight for edge cases
- [OK] Continuous learning from decisions
- [OK] Fast processing with cost optimization
- [OK] Complete audit trail
- [OK] User appeal mechanism
