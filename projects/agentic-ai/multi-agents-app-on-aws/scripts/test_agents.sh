...
if [ -n "$RESULT" ]; then
    echo -e "${GREEN}[OK] Research Agent works locally${NC}"
else
    echo -e "${RED}[FAIL] Research Agent failed${NC}"
fi
...
if [ -n "$RESULT" ]; then
    echo -e "${GREEN}[OK] Analyst Agent works locally${NC}"
else
    echo -e "${RED}[FAIL] Analyst Agent failed${NC}"
fi
...
if [ -n "$RESULT" ]; then
    echo -e "${GREEN}[OK] Full pipeline works locally${NC}"
    echo ""
    echo "Sample output (last 10 lines):"
    echo "$RESULT"
else
    echo -e "${RED}[FAIL] Orchestrator pipeline failed${NC}"
fi
...
if command -v agentcore &> /dev/null; then
    ...
    if [ -n "$DEPLOYED" ]; then
        echo -e "${GREEN}[OK] Deployed agent responds${NC}"
    else
